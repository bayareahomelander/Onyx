...
from typing import Optional, Generator, List, Tuple, Union, Any
import time
...
def _normalize_stop_sequences(stop_tokens: Optional[List[Any]]) -> List[List[int]]:
    if not stop_tokens:
        return []

    sequences: List[List[int]] = []
    for item in stop_tokens:
        if isinstance(item, int):
            sequences.append([item])
        elif isinstance(item, (list, tuple)):
            sequence = [token for token in item if isinstance(token, int)]
            if sequence:
...
def _has_stop_suffix(tokens: List[int], stop_sequences: List[List[int]]) -> bool:
    return _matched_stop_sequence(tokens, stop_sequences) is not None


def _trim_stop_suffix(tokens: List[int], stop_sequences: List[List[int]]) -> List[int]:
    matched = _matched_stop_sequence(tokens, stop_sequences)
    if matched is None:
        return tokens
...
class SpeculativeEngine:
    ...
    def generate(
        ...
        max_tokens: int = 50,
        gamma: int = 4,
        stop_tokens: Optional[List[Any]] = None,
        ...
        args:
            ...
            stop_tokens: Token IDs or token ID sequences that stop generation
        ...
        stop_sequences = _normalize_stop_sequences(stop_tokens)
        ...
        if finish_reason is None:
            while len(generated_tokens) < max_tokens:
                ...
                for _ in range(draft_budget):
                    ...
                    if _has_stop_suffix(generated_tokens + draft_tokens, stop_sequences):
                        break
