# Onyx technical report

Architecture, API details, examples, and recorded benchmarks for the Apple
Silicon implementation. For installation, see the [README](README.md). The
Windows implementation has its own [technical report](onyx_cuda/REPORT.md) and
[reproducibility guide](onyx_cuda/REPORT.md#repeatable-windows-delivery-check).

Benchmarks below use Apple Silicon and 4-bit quantized Qwen models. Results are
specific to the measured hardware and workloads; a token budget can leave
...
- **Grammar Compliance**: All outputs in the reported Apple Silicon benchmark cases matched their constraints
...
- **Supported JSON Schema Subset**: Nested objects, typed arrays, regex patterns, enums, unions, and length constraints

---

## JSON Schema Constraints

Onyx includes a structure-aware JSON generation engine built in Rust. It applies
supported schema constraints at the token level. Interrupted or budget-limited
generation may be incomplete; the [CUDA contract](onyx_cuda/REPORT.md#structured-output-contract)
explicitly distinguishes validated completion from partial output.
...
Enforce regex patterns during generation. Budget-limited output may be incomplete.
