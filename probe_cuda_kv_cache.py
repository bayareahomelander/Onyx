"""Run the bounded real-model CUDA KV-cache probe."""
...
from onyx_cuda.kv_cache_probe import (
    DEFAULT_MODEL_ID,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Load the pinned INT4 Qwen model, run prefill plus one cached decode "
            "step, and validate KV-cache growth, placement, and cleanup."
        )
    )
    parser.add_argument(
        "--revision",
    ...
    parser.add_argument(
        "--prompt", default=DEFAULT_PROMPT, help="Prompt for the bounded cache probe"
    )
    parser.add_argument(
        "--regex",
        default=DEFAULT_REGEX,
        help="One-token constraint used to choose the cached decode input",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        print_console_safe(f"CUDA KV-cache probe could not run: {type(exc).__name__}: {exc}")
        return 2

    print_console_safe(format_kv_cache_report(report))
